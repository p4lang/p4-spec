...
\lstdefinestyle{P4style}{
    ...
    keywords={action, action_function_declaration, action_profile, action_selector, algorithm, and, apply, attribute, attributes, bit, bytes, calculated_field, control, counter, direct, direct_attibute, dynamic_action_selection, else, extern, extern_type, extract, false, field_list, field_list_calculation, fields, header, header_type, hit, if, in, inout, input, instance_count, int, last, layout, mask, max, metadata, meter, method, min, min_width, miss, next, not, optional, or, out, output_width, packets, parse_error, parser, parser_drop, parser_exception, parser_value_set, range, register, result, return, saturating, select, selection_key, set_metadata, signed, static, table, true, typedef, update, valid, value, varbit, verify, void, width},%
    basicstyle=\ttfamily,%
    aboveskip=3mm,%
    belowskip=3mm,%
    fontadjust=true,%
...
Section~\ref{sec:xxx} for more information.
...
Bit strings with a width of 0 are used for action and method parameters which are
agnostic to the bit width of the arguments bound to them, but require a finite-width 
bitstring all the same. It is an error to use this except for a parameter
list or an attribute declaration of an extern type, since it implies a fixed-width 
bitstring whose width is inferred at compile-time.
...
\underline{TODO: import Mihai's spec here.}
...
Here is the specification for the standard primitive actions.
 ...
 * 
 * Update the indicated field's value. 
 *
 * The dst parameter may be:
 *   - A field from the parsed representation (\texttt{field_ref}).
 *   - A register reference (\texttt{register_ref}).
 *
 * The src parameter may be:
 ...
 *   - A field from the parsed representation (\texttt{field_ref}).
 *   - A register reference (\texttt{register_ref}).
 *   - An arithmetic expression involving any of the above (\texttt{general_expr}).
 ...
 * Set the value of a field using a calculated hash value and ...
 * Representation
 * 
 * @param dst    The name of the field instance to modify.
 * @param hash   The field list calculation to use to generate the hash value.
 * @param base   The base value to use for the index.
 * @param size   The maximum value to use for the index if > 0.
 *
 * The dst parameter may be:
 *   - A field from the parsed representation (\texttt{field_ref}).
 *   - A register reference (\texttt{register_ref}).
 *
 * Both the base and the size parameters are:
 *   - An immediate value (a number).
 *   - A value from the matching entry's action parameter data;
 *      in this case, the name of a parameter from the enclosing
 *      function is used.
 * 
 * Perform the following calculation and write it into the destination.
 *
...
action set_field_to_hash_index(out bit<0> dst, in field_list_calc hash,
                               optional in int base, optional in int size);
...
action push(out header array[], optional in int count);
...
action pop(out header array[], optional in int count);
 ...
 * Update a counter.
 *
 * @param cntr   The name of the counter array (\texttt{counter_name}).
 * @param index  The offset in the array to get a counter reference.
 *
...
action count(in counter cntr, in int index);
...
/**
 * meter
 * Execute a metering operation.
 *
 * @param mtr    The name of the meter array (\texttt{meter_name}).
 * @param index  The offset in the array to get a meter reference.
 * @param field  The field reference to store the meter state (\texttt{field_ref}).
 *
 * The given meter reference, determined by mtr and index, is executed. If the 
 * meter is direct, then index is ignored as the table entry determines which 
 ...
 * stored in field. If the parent header of field is not valid, the meter 
 * state is updated, but the state of the meter is discarded.
 **/

action meter(in meter mtr, in int index, in bit<0> field);
 ...
 * 
 * @param receiver  An opaque value identifying the receiver.
 * @param digest    A list of field references (\texttt{field_list_name}).
 *
...
action generate_digest(in int receiver, in field_list digest);
...
/**
 * resubmit
 * Applied in the ingress pipeline, mark the packet to be resubmitted to the parser.
 *
 * @param digest  An optional list of metadata field references (\texttt{field_list_name)}.
 *
 ...
 * will be resubmitted to the parser with values of the fields in digest from 
...
action resubmit(optional in field_list digest);
 ...
 * with values of the fields in digest from the processing on the packet. 
...
action recirculate(optional in field_list digest);
 ...
 *
 * @param clone_spec  An opaque identifier indicating additional run time 
 *                    characteristics of the clone operation.
 * @param digest      An optional list of metadata field references (\texttt{field_list_name}).
 * 
 ...
 * been produced.
 *
 * The clone_spec is used to allow the configuration of other target specific 
 ...
 * ingress clone. 
 *
 * The fields indicated in digest are copied to the Parsed Representation 
...
action clone_ingress_pkt_to_ingress(in int clone_spec, optional in field_list digest);
 ...
 * Generate a copy of the egress packet and submit it to the ingress parser.
 *
 * @param clone_spec  An opaque identifier indicating additional run time 
 *                    characteristics of the clone operation.
 * @param digest      An optional list of metadata field references (\texttt{field_list_name}).
...
action clone_egress_pkt_to_ingress(in int clone_spec, optional in field_list digest);
 ...
 * Generate a copy of the original packet and submit it to the Buffering Mechanisms.
 *
 * @param clone_spec  An opaque identifier indicating additional run time 
 *                    characteristics of the clone operation.
 * @param digest      An optional list of metadata field references (\texttt{field_list_name}).
 ...
 * The clone_spec is used to allow the configuration of other target specific 
 * characteristics of the clone operation as described in \texttt{clone_ingress_pkt_to_ingress}. 
 * In addition to other session attributes, clone_spec determines the egress 
...
action clone_ingress_pkt_to_egress(in int clone_spec, optional in field_list digest);
 ...
 * Generate a copy of the egress packet and submit it to the Buffering Mechanisms.
 *
 * @param clone_spec  An opaque identifier indicating additional run time 
 *                    characteristics of the clone operation.
 * @param digest      An optional list of metadata field references (\texttt{field_list_name}).
 ...
 * fields specified in digest are submitted to the Buffering Mechanism as 
...
action clone_egress_pkt_to_egress(in int clone_spec, optional in field_list digest);
...
\underline{TODO: DO WE NEED THIS SECTION?}
...
\SUBSUBSECTION{Execution Semantics}{executionsemantics}

\underline{TODO: Add Anirudh's proposal here.}
...
attribute_declaration ::= 
    attribute attribute_name {
        type : attribute_type ;
        [ optional ; ]
        [ local_variables : { identifier_list+ } ]
    }
...
and variable declarations, an attribute type may also be \textit{void}.
In the latter case, the attribute is immediately followed by a semicolon 
and has no value other than its presence (or lack of absence).
...
\textit{local_variables} property (in addition to P4 identifiers
...
extern type supplied the target provider is responsible for explaining when 
