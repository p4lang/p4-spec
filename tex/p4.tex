...
\lstdefinestyle{P4style}{
    ...
    keywords={action, action_function_declaration, action_profile, action_selector, algorithm, and, apply, attribute, attributes, bit, bytes, calculated_field, control, counter, direct, direct_attibute, dynamic_action_selection, else, extern, extern_type, extract, false, field_list, field_list_calculation, fields, header, header_type, hit, if, in, inout, input, instance_count, int, last, layout, mask, max, metadata, meter, method, min, min_width, miss, next, not, optional, or, output_width, packets, parse_error, parser, parser_drop, parser_exception, parser_value_set, primitive_action, range, register, result, return, saturating, select, selection_key, set_metadata, signed, static, table, true, typedef, update, valid, value, varbit, verify, void, width},%
    basicstyle=\ttfamily,%
    aboveskip=3mm,%
    belowskip=3mm,%
    fontadjust=true,%
...
{ % Parameters
\paramdoc{dest}{(FLD or R-REF) The name of the field instance to be modified.}
\paramdoc{value}{(VAL, FLD or R-REF) The value to use.}
}
{ % Description
The \texttt{dest} field's value is updated by adding the value parameter. The \texttt{value} parameter 
may be from a table parameter, an immediate a value, a field reference or 
a register reference; see \texttt{modify_field} above.  If \texttt{value} is a field reference 
...
{ % Parameters
\paramdoc{dest}{(FLD or R-REF) The name of the field instance to be modified.}
\paramdoc{value1}{(VAL, FLD or R-REF) The first value to use.}
\paramdoc{value2}{(VAL, FLD or R-REF) The second value to use.}
}
{ % Description
...
\primact{set_field_to_hash_index(dest, field_list_calc, base, size)}
{ % Summary
Add a value to a field.
}
{ % Parameters
...
param_qualifier ::= in | inout | optional
