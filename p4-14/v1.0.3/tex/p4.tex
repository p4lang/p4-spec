...
{\color{blue} Counters, meters and registers are referenced in special primitive actions as defined 
...
counters. {\color{blue} Registers are declared with a width
...
\texttt{\color{blue}modify_field_with_hash_based_offset} &
...
\texttt{\color{blue}execute_meter} &
Execute a meter operation. \\ \hline
\texttt{\color{blue}register_read} &
{\color{blue}Read from an indexed instance of a register and store the value into a field.} \\ \hline
\texttt{\color{blue}register_write} &
{\color{blue}Write a value into an indexed instance of a register. }\\ \hline
...
{ % Parameters
\paramdoc{dest}{{\color{blue}(FLD) The name of the field instance to modify (destination).}}
\paramdoc{value}{{\color{blue}(VAL or FLD) The value to use (source).}}
...
{ % Parameters
\paramdoc{dest}{{\color{blue}(FLD) The name of the field instance to be modified.}}
\paramdoc{value}{{\color{blue}(VAL or FLD) The value to use.}}
}
{ % Description
The \texttt{dest} field's value is updated by adding the value parameter. The \texttt{value} parameter 
may be from a table parameter, an immediate value {\color{blue}or a field reference}; see \texttt{modify_field} above.  If \texttt{value} is a field reference 
...
table parameter, an immediate a value {\color{blue}or a field reference}; see \texttt{modify_field} above.  If either \texttt{value}
...
\primact{{\color{blue}modify_field_with_hash_based_offset(dest, base, field_list_calc, size)}}
{ % Summary
{\color{blue}Apply a field list calculation to compute a hash value.}
}
{ % Parameters
...
\paramdoc{size}{(VAL)  \color{blue} The size of the hash value range. Must be larger than 0.}
}
{ % Description
...
\primact{\color{blue}push(array, count)}
{ % Summary
...
\paramdoc{count}{(VAL) \color{blue}A value indicating the number of elements to push.}
}
{ % Description
...
{\color{blue}Existing elements will be shifted by \texttt{count}: an
...
\primact{\color{blue}pop(array, count)}
{ % Summary
Pop header instances from the top of an array, moving all subsequent array elements up.
}
{ % Parameters
\paramdoc{array}{(ARR) The name of the instance array to be modified.}
\paramdoc{count}{(VAL) \color{blue}A value indicating the number of elements to pop.}
}
{ % Description
This primitive is used to remove elements from an array of header instances
without knowing in advance how many elements are already valid. {\color{blue}An element at
...
\primact{\color{blue}execute_meter(meter_ref, index, field)}
{ % Summary
...
\primact{\color{blue}resubmit(field_list)}
{% textbf  
Applied in the ingress pipeline, mark the packet to be resubmitted to the 
parser.
}
{ % Parameters
\paramdoc{field_list}{(FLDLIST) \color{blue}A list of metadata field references.} 
}
{ % Description
...
\primact{\color{blue}recirculate(field_list)}
{ % Summary
...
The packet is marked for  {\color{blue}recirculation}. It will complete the egress pipeline 
and be deparsed.  This version of the packet is then {\color{blue}recirculated} to the parser 
...
\primact{\color{blue}clone_ingress_pkt_to_ingress(clone_spec, field_list)}
{ % Summary
...
\primact{\color{blue}clone_egress_pkt_to_ingress(clone_spec, field_list)}
{ % Summary
...
\primact{\color{blue}clone_ingress_pkt_to_egress(clone_spec, field_list)}
{ % Summary
...
\primact{\color{blue}clone_egress_pkt_to_egress(clone_spec, field_list)}
{ % Summary
...
{\color{blue}
1.0.3 & \color{blue} 2016-09-13 & \color{blue} Added \texttt{register_read} and \texttt{register_write} primitive actions and removed reigster layout and bracket-based access method. Replaced \texttt{meter} primitive action by \texttt{execute_meter}. Replaced \texttt{set_field_to_hash_index} primitive action by \texttt{modify_field_with_hash_based_offset}. Changed optional parameters of \texttt{push, pop, resubmit, recirculate, clone_*} primitive actions to mandatory parameters.
}
\end{tabular}
\end{center}
