...
\SUBSECTION{\color{red}Counters}{counters}
...
counter_type ::= @\textbf{bytes}@ | @\textbf{packets}@ | bytes_and_packets
...
\SUBSECTION{\color{red}Meters}{meters}
...
{ % Parameters
\paramdoc{dest}{{\color{red}(FLD) The name of the field instance to modify (destination).}}
\paramdoc{value}{{\color{red}(VAL or FLD) The value to use (source).}}
...
{ % Parameters
\paramdoc{dest}{{\color{red}(FLD) The name of the field instance to be modified.}}
\paramdoc{value}{{\color{red}(VAL or FLD) The value to use.}}
}
{ % Description
...
\primact{\color{red}subtract_from_field(dest, value)}
{ % Summary
...
\primact{\color{red}subtract(dest, value1, value2)}
{ % Summary
...
\primact{{\color{red}modify_field_with_hash_based_offset(dest, base, field_list_calc, size)}}
{ % Summary
...
\paramdoc{size}{(VAL)  \color{red} The size of the hash value range. Must be larger than 0.}
}
{ % Description
...
\primact{{\color{red}modify_field_rng_uniform(dest, lower_bound, upper_bound)}}
{ % Summary
...
\primact{\color{red}bit_and(dest, value1, value2)}
{ % Summary
...
\primact{\color{red}bit_or(dest, value1, value2)}
{ % Summary
...
\primact{\color{red}bit_xor(dest, value1, value2)}
{ % Summary
...
\primact{\color{red}shift_left(dest, value1, value2)}
{ % Summary
...
\primact{\color{red}shift_right(dest, value1, value2)}
{ % Summary
...
\primact{\color{red}push(array, count)}
{ % Summary
...
\paramdoc{count}{(VAL) \color{red}A value indicating the number of elements to push.}
}
{ % Description
...
\primact{\color{red}pop(array, count)}
{ % Summary
Pop header instances from the top of an array, moving all subsequent array elements up.
}
{ % Parameters
\paramdoc{array}{(ARR) The name of the instance array to be modified.}
\paramdoc{count}{(VAL) \color{red}A value indicating the number of elements to pop.}
}
{ % Description
...
\primact{\color{red}execute_meter(meter_ref, index, field)}
{ % Summary
...
\primact{\color{red}resubmit(field_list)}
{% textbf  
Applied in the ingress pipeline, mark the packet to be resubmitted to the 
parser.
}
{ % Parameters
\paramdoc{field_list}{(FLDLIST) \color{red}A list of metadata field references.} 
}
{ % Description
...
\primact{\color{red}recirculate(field_list)}
{ % Summary
...
\primact{\color{red}clone_ingress_pkt_to_ingress(clone_spec, field_list)}
{ % Summary
...
\primact{\color{red}clone_egress_pkt_to_ingress(clone_spec, field_list)}
{ % Summary
...
\primact{\color{red}clone_ingress_pkt_to_egress(clone_spec, field_list)}
{ % Summary
...
\primact{\color{red}clone_egress_pkt_to_egress(clone_spec, field_list)}
{ % Summary
...
\SUBSUBSECTION{\color{red}Summary of changes introduced in 1.0.3}{change-summary-1.0.3}

\begin{itemize}

\item Counters and meters (Sections \ref{sec:counters} and \ref{sec:meters})
  \begin{itemize}
  \item A new counter type \texttt{bytes_and_packets}.
