...
will be resubmitted to the parser with values of the fields in \texttt{field_list}
that those fields have at the time ingress processing completes.
These values replace the normal initial
values of the metadata fields indicated in the initializer of the instance 
declaration. Only metadata (and not header) fields can be specified in the \texttt{field_list}.

If multiple resubmit actions get executed on one packet, only the
field list from the last resubmit action is used, and only one packet
is resubmitted.

The resubmitted packet will have \texttt{instance_type} set to
indicate that it is a resubmitted packet.
...
with values of the fields in \texttt{field_list} that those fields
have at the time egress processing completes.
...
If multiple recirculate actions get executed on one packet, only the
field list from the last recirculate action is used, and only one
packet is recirculated.

The recirculated packet will have \texttt{instance_type} set to
indicate that it is a recirculated packet.
...
The metadata fields indicated in \texttt{field_list} (their values when ingress processing is complete) are copied to the Parsed Representation 
...
values (the values at the end of egress processing) replace the normal initial values of the metadata fields indicated 
in the initializer of the instance declaration. Only metadata (and not header) fields can be specified in the \texttt{field_list}.
...
fields specified in \texttt{field_list} (their values at the end of egress processing) are submitted to the Buffering Mechanism as 
