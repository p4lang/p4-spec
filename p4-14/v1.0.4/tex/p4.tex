...
\vspace{3mm}
\centerline{\sffamily\large March XX, 2017}
\vspace{8mm}
...
When an invalid packet header is matched in a table (i.e., when a value is
specified for exact or ternary matches against the field), its value is
undefined. A match operation may explicitly check if a header
...
1.0.4 & 2017-MM-DD & See \ref{sec:change-summary-1.0.4} for details. \\ \hline
}
\end{tabular}
\end{center}
...
\begin{itemize}
\item The value of an invalid header is undefined when used for matching in a table.
\end{itemize}
\end{itemize}
